...
SynonymSwing:
SynonymSwing is a Java-based text generator that uses Concurrent Hash Maps to analyze a given text,
Save and generate new paragraphs based on the frequency of word occurrences.
It utilizes the LinkedList API to store unique keywords and their associated following words,
allowing the user to generate random paragraphs with a similar structure to the analyzed text.

Features:
Analyzes input text and stores word occurrences in a linked list structure
...
MainLinkedList: Contains all the MainLink instances and manages the insertion and generation of text.
